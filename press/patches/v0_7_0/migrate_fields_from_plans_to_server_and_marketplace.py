...
def execute():
	plans = frappe.get_all("Site Plan", {"document_type": ("in", ["Server", "Database Server"])}, pluck="name")
	...
	for marketplace_plan in frappe.get_all("Marketplace App Plan", pluck="name"):
		...
		plan = frappe.get_all("Plan", {"name": map_doc.plan}, ["plan_title", "price_usd", "price_inr"])
