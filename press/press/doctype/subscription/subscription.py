...
class Subscription(Document):
	def before_insert(self):
		...
		customer_id, currency, payment_method = frappe.db.get_value(
			"Team",
			self.team,
			["stripe_customer_id", "transaction_currency", "stripe_default_payment_method_id"],
		)
		# get plan id from plan
		plan_id_field = "stripe_inr_plan_id" if currency == "INR" else "stripe_usd_plan_id"
		plan_id = frappe.db.get_value("Plan", self.plan, plan_id_field)
