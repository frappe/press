...
class Invoice(Document):
	...
	def finalize_invoice(self):  # noqa: C901
		...
		if self.amount_due > 0:
			...
			elif self.payment_mode == "Razorpay Mandate":
	...
	def _make_razorpay_payment(self, mandate, amount):
		...
		try:
			...
			payment_data = {
				...
				"contact": frappe.db.get_value("Team", self.team, "phone") or "",
				"amount": amount,
				"currency": "INR",
				"order_id": self.razorpay_order_id if self.razorpay_order_id else None,
			...
			payment = client.payment.create_recurring_payment(data=payment_data)
