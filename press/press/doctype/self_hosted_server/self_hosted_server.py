...
class SelfHostedServer(Document):
	...
	def fetch_apps_and_sites(self):
		frappe.enqueue_doc(
			self.doctype, self.name, "_get_apps", queue="long", timeout=1200
		)
		frappe.enqueue_doc(
			self.doctype, self.name, "_get_sites", queue="long", timeout=1200
		)
	...
	def _get_old_sites(self):
		...
		"""
		try:
			ansible = Ansible(playbook="get_sites.yml",server=self,user=self.ssh_user,
			variables={
	...
	def _get_old_apps(self):
		...
		"""
		try:
			ansible = Ansible(playbook="get_apps.yml",server=self,user=self.ssh_user,
			variables={
				"bench_path":self.bench_directory
			})
			play = ansible.run()
	...
	def append_to_sites(self):
		...
		ansible_task_op = frappe.get_value("Ansible Task",{"play":ansible_play.name,"task":"Get Sites from Current Bench"},"output")
	...
	def append_to_apps(self):
		...
		"""
		ansible_play = frappe.get_last_doc("Ansible Play",{"server":self.name})
		ansible_task_op = frappe.get_value("Ansible Task",{"play":ansible_play.name,"task":"Get Versions from Current Bench"},"output").replace("\'","\"")
	...
	def create_new_rg(self):
		...
		ansible_task_op = frappe.get_value("Ansible Task",{"play":ansible_play.name,"task":"Get Apps for Release Group"},"output").replace("\'","\"")
			...
			for app in task_result:
				...
				if not frappe.db.exists("App Source",{"app":app['app'],"branch":app['branch']}):
					if not frappe.db.exists("App",{"_newname":app['app']}):
						...
						app_source_doc.append("versions",{"version": self.map_branch_to_version(max(branches))})
						app_source_doc.insert()
						frappe.db.commit()
						release_group.append("apps",{
							"app":app['app'],
							"source": app_source_doc.name
						})
				release_group.append("apps",{
					"app":app['app'],
					"source": frappe.get_value("App Source",{
						"app":app['app'],
						"branch":app['branch'],
					},"name")})
		except Exception:
	...
	def append_site_configs(self,play_name):
		...
		"""
		ansible_task_op = frappe.get_value("Ansible Task",{"play":play_name,"task":"Get Site Configs from Existing Sites"},"output").replace("\'","\"")
		task_result = json.loads(ansible_task_op.replace("\"{","{").replace("}\"","}").replace("\\n",""))
