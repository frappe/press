...
class TestAgentJob(FrappeTestCase):
	...
	def test_publish_update_publishes_realtime_events(self, mock_publish):
		site = create_test_site()
		...
		from press.press.doctype.agent_job.agent_job import publish_update

		publish_update(job.name)

		self.assertTrue(mock_publish.called)

		events = [call.kwargs.get("event") for call in mock_publish.call_args_list]

		self.assertIn("agent_job_update", events)
		self.assertIn("doc_update", events)
		self.assertIn("list_update", events)

		mock_publish.assert_any_call(
			event="agent_job_update",
			doctype="Agent Job",
			docname=job.name,
			message=Mock(),
		)
		...
		mock_publish.assert_any_call(
			...
			doctype="Agent Job",
			docname=job.name,
			message={"doctype": "Agent Job", "name": job.name},
		)
		...
		mock_publish.assert_any_call(
			...
			doctype="Agent Job Step",
			docname=step.name,
			message={"doctype": "Agent Job Step", "name": step.name},
		)
