...
class PressPermissionGroup(Document):
            ...
            for doc_name, doc_perms in doctype_perms.items():
                ...
                for method, permitted in doc_perms.items():
                    if method != "*" and method not in restrictable_methods:
                        frappe.throw(
                            f"{method} is not a restrictable method of {doctype}"
                        )
    ...
    def get_all_document_permissions(self, doctype: str) -> list:
        ...
        user_is_team_owner = frappe.db.exists("Team", {"name": self.team, "user": user})
        if (
            not user_belongs_to_group
            and user != "Administrator"
            and not user_is_team_owner
        ):
            frappe.throw(f"{user} does not belong to {self.name}")
        ...
        for doc in docs:
            permitted_methods = get_permitted_methods(
                doctype, doc.name, group_names=[self.name]
            )
            doc_perms = []
...
def get_permitted_methods(doctype: str, name: str, group_names: list = None) -> list:
    ...
    permitted_methods = [
        method for method, permitted in method_perms.items() if permitted
    ]
