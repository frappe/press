...
class Cluster(Document):
	...
	def _create_auto_cluster(self):
		"""Provisions a new independent cluster cloned from this one.

		Called as a background job when ``get_running_vm_count()`` reaches
		``max_servers``.  The new cluster is fully self-contained (no parent
		link) and inherits ``enable_auto_cluster`` / ``max_servers`` so it
		will itself spawn the next cluster when it fills up.

		Naming convention: ``mumbai`` -> ``mumbai-1`` -> ``mumbai-2``; a
		re-triggered ``mumbai`` (after some children were archived) produces
		``mumbai-3`` because the global max across all children is used.
		"""
		# Guard: if the flag was cleared between the enqueue and now, bail out.
		if not frappe.db.get_value("Cluster", self.name, "auto_cluster_triggered"):
			return

		# The root cluster is `self` when this is the original cluster, or the
		# value of `derived_cluster_from` when this cluster is itself
		# auto-created.  All auto-generated siblings store the *same* root name,
		# so counting them is a single indexed DB query instead of a regex scan.
		root_name = self.derived_cluster_from or self.name

		# Find the highest sequential suffix already used under this root.
		...
		new_n = len(children) + 1
		new_name = f"{root_name}-{new_n}"
		new_title = f"{self.title or root_name} {new_n}"
		...
		cluster_data: dict = {
			...
			# Plans
			"default_app_server_plan": self.default_app_server_plan,
			"default_db_server_plan": self.default_db_server_plan,
			"default_app_server_plan_type": self.default_app_server_plan_type,
			"default_db_server_plan_type": self.default_db_server_plan_type,
			"by_default_select_unified_mode": self.by_default_select_unified_mode,
			# Features
			"has_arm_support": self.has_arm_support,
			"has_unified_server_support": self.has_unified_server_support,
			"has_add_on_storage_support": self.has_add_on_storage_support,
			"enable_autoscaling": self.enable_autoscaling,
			"enable_periodic_flush_table": self.enable_periodic_flush_table,
			"flush_table_execution_hour": self.flush_table_execution_hour,
			"disable_public_ips_for_servers": self.disable_public_ips_for_servers,
			# Auto-cluster (inherited so the new cluster chains when it fills up)
			"enable_auto_cluster": self.enable_auto_cluster,
			"max_servers": self.max_servers,
			# Parent reference - always points to the original root cluster so
			# that siblings share a single `derived_cluster_from` value and
			# can be counted with a single indexed query.
			"derived_cluster_from": root_name,
			# App-server count is configured on the root; children inherit it.
			"auto_cluster_app_server_count": self.auto_cluster_app_server_count or 2,
		}

		# Copy provider-specific credentials
		if self.cloud_provider == "AWS EC2":
			cluster_data["aws_access_key_id"] = self.aws_access_key_id
			cluster_data["aws_secret_access_key"] = self.get_password("aws_secret_access_key")
		elif self.cloud_provider == "Hetzner":
			cluster_data["hetzner_api_token"] = self.get_password("hetzner_api_token")
		elif self.cloud_provider == "OCI":
			cluster_data["oci_user"] = self.oci_user
			cluster_data["oci_tenancy"] = self.oci_tenancy
			cluster_data["oci_public_key"] = self.oci_public_key
			cluster_data["oci_private_key"] = self.get_password("oci_private_key")
		elif self.cloud_provider == "DigitalOcean":
			cluster_data["digital_ocean_api_token"] = self.get_password("digital_ocean_api_token")
		elif self.cloud_provider == "Frappe Compute":
			cluster_data["frappe_compute_base_url"] = self.frappe_compute_base_url
			cluster_data["frappe_compute_api_key"] = self.frappe_compute_api_key
			cluster_data["frappe_compute_api_secret"] = self.get_password("frappe_compute_api_secret")

		new_cluster: Cluster = frappe.get_doc(cluster_data).insert(ignore_permissions=True)
		frappe.db.commit()

		new_cluster.reload()

		# Images are typically already available when the new cluster shares
		# the same region.  Fall back to copying if not.
		if new_cluster.images_available < 1:
			new_cluster._add_images()
			frappe.db.commit()
			new_cluster.reload()

		# Provision: 1 Proxy + 2 public (App + DB) server pairs.
		new_cluster._provision_auto_cluster_servers()
		frappe.db.commit()

	def _provision_auto_cluster_servers(self):
		"""Creates the initial servers for an auto-created cluster.

		Creates 1 Proxy Server and N public App + Database Server pairs,
		where N is ``auto_cluster_app_server_count`` (default 2, set on the
		root cluster and inherited by all children).

		The App Servers are marked ``public = True`` so they are available
		for site / bench placement immediately.  The Database Server's
		``public`` flag is synced automatically by the Server ``on_update``
		hook when the linked App Server is saved.
		"""
		num_pairs = self.auto_cluster_app_server_count or 1

		app_plan = (
			frappe.get_doc("Server Plan", self.default_app_server_plan)
			if self.default_app_server_plan
			else None
		)
		db_plan = (
			frappe.get_doc("Server Plan", self.default_db_server_plan)
			if self.default_db_server_plan
			else None
		)

		# --- 1. Proxy Server ------------------------------------------------
		proxy, _ = self.create_server("Proxy Server", DEFAULT_SERVER_TITLE)
		self.proxy_server = proxy.name
		frappe.db.commit()

		# --- 2. N public App + DB pairs -------------------------------------
		for pair_index in range(1, num_pairs + 1):
			# Database Server for this pair
			db_server, _ = self.create_server(
				"Database Server", f"Public DB Server {pair_index}", plan=db_plan
			)
			self.database_server = db_server.name

			# App Server linked to the DB and proxy above, marked public
			self.create_server(
				"Server",
