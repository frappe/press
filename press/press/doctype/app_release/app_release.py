...
class AppRelease(Document):
	...
	dashboard_fields: ClassVar[list] = ["app", "source", "message", "hash", "author", "status"]
...
def check_python_syntax(dirpath: str) -> str:
	...
	python_exec = _get_python()

	command = f"{python_exec} -m compileall -q -o 0 {dirpath}"
	proc = subprocess.run(
		shlex.split(command),
		text=True,
		capture_output=True,
...
def _get_python():
	...
	python_exec = f"{bench_path}/env/bin/python3"
