...
# Errors that require cancelling the mandate — token is dead, cannot recover automatically
_MANDATE_CANCEL_ERRORS = {"TOKEN_EXPIRED", "TOKEN_NOT_ACTIVE"}
...
def razorpay_authorized_payment_handler():
	...
	try:
		...
		# Skip silently if this is not a prepaid credits order (e.g. UPI mandate authorization)
		# to avoid crashing on missing signature for non-prepaid events
...
def razorpay_webhook_handler():
	...
	try:
		...
		# Skip silently if this is not a prepaid credits order (e.g. UPI mandate events)
		# to avoid crashing on missing signature for non-prepaid events
...
def _handle_token_confirmed(form_dict):
	...
	if mandate_id:
		# Find by mandate_id stored in notes (check Pending first, then Active)
		mandate = frappe.db.get_value(
			"Razorpay Mandate",
			{"mandate_id": mandate_id, "status": ("in", ["Pending", "Active"])},
			"name",
		)
	else:
		...
		mandate = frappe.db.get_value(
			"Razorpay Mandate",
			{"razorpay_customer_id": customer_id, "status": ("in", ["Pending", "Active"])},
			"name",
		)
	...
	# Skip if mandate is already activated (frontend callback already processed it)
	if mandate_doc.status == "Active" and mandate_doc.token_id:
		return
...
def _handle_recurring_payment_captured(form_dict):
	...
	invoice = frappe.get_doc("Invoice", invoice_name, for_update=True)

	# Idempotency: skip if invoice is already submitted and paid
	if invoice.docstatus == 1 and invoice.status == "Paid":
		return
...
def _handle_recurring_payment_failed(form_dict):
	...
	if error_reason in _MANDATE_CANCEL_ERRORS:
		# Token is dead — cancel the mandate so the user sets up a fresh one
		mandate = frappe.db.get_value(
			"Razorpay Mandate",
			{"team": invoice.team, "is_default": 1, "status": "Active"},
			"name",
		)
		if mandate:
			frappe.get_doc("Razorpay Mandate", mandate).cancel()

		invoice.razorpay_payment_id = None
		invoice.razorpay_order_id = None
		invoice.status = "Unpaid"
		invoice.save(ignore_permissions=True)
		team.send_email_for_failed_upi_payment(invoice, error_reason=error_reason, upi_vpa=upi_vpa)

	else:
		# For all other errors: do NOT reset the invoice here. A payment.failed webhook
		# fires on the initial rejection but payment.captured can still follow if the
		# customer retries within their UPI app. The hourly finalize_razorpay_mandate_invoices
		# job polls Razorpay and resets to Unpaid only when the API confirms "failed"
		# (after the capture window is definitively closed). No automatic retry is scheduled
		# — re-debiting via a monthly mandate risks FREQUENCY_LIMIT_EXCEEDED.
		invoice.save(ignore_permissions=True)
		team.send_email_for_failed_upi_payment(invoice, error_reason=error_reason, upi_vpa=upi_vpa)
