def frappecloud_migrator(local_site):
	...
	subprocess.call(shlex.split("{} -m pip install {}".format(sys.executable, " ".join(dependencies))), stdout=subprocess.DEVNULL)
	...
	@tenacity.retry(stop=tenacity.stop_after_attempt(2), wait=tenacity.wait_fixed(5))
