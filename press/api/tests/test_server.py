...
class TestServerPlansForUpgrade(FrappeTestCase):
	...
	def test_has_similar_enabled_plans_ignores_legacy_plans(self):
		...
		self.assertFalse(has_similar_enabled_plans("Database Server", "arm64", self.cluster.name))

		self._plan(platform="arm64", legacy_plan=False)

		self.assertTrue(has_similar_enabled_plans("Database Server", "arm64", self.cluster.name))
