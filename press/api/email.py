...
def validate_plan(team, site):
    ...
    """
    # ToDo: Authentication
    active = frappe.db.get_all('QMail Subscription', pluck="emails", filters={'team': team, 'site': site, 'enabled': 1})

    if active and len(active) == 1:
        date = datetime.today().date()
        first_day = date.strftime("%Y-%m-01")

        count = frappe.db.count('QMail Log', filters={'site': site, 'status': 'Sent', 'date': ['>=', first_day]})
