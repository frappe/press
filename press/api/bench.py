...
def new(bench):
	...
	if not team.enabled:
		frappe.throw("Your account is currently disabled. Please re-enable your account to create a new bench.")

	if exists(bench["title"]):
		frappe.throw("A bench with this name already exists on your account. Please choose a different name.")
	...
	if bench["server"] and not (
		...
		frappe.throw("You can only create benches on servers owned by your team. <a href='https://docs.frappe.io/cloud/change-team-of-bench-group' target='_blank'>Learn more</a>")
...
def options():
	...
	if not versions:
		frappe.throw("No app versions found. Only enabled and public app sources are listed here. Please check your app source settings.")
...
def update_dependencies(name: str, dependencies: str):
	...
	if len(rg.dependencies) != len(dependencies):
		frappe.throw("All required dependencies must be provided. <a href='https://docs.frappe.io/cloud/common-issues/required-app-not-found' target='_blank'>Learn more</a>")
	if diff := set([d["key"] for d in dependencies]) - set(d.dependency for d in rg.dependencies):
		frappe.throw("Invalid dependencies: " + ", ".join(diff) + ". <a href='https://docs.frappe.io/cloud/common-issues/incompatible-dependency-version' target='_blank'>Learn more</a>")
	for dep, new in zip(
		...
		if dep.dependency != new["key"]:
			frappe.throw(f"Invalid dependency: {new['key']}. <a href='https://docs.frappe.io/cloud/common-issues/incompatible-dependency-version' target='_blank'>Learn more</a>")
		if not re.match(r"^\d+\.\d+\.*\d*$", new["value"]):
			frappe.throw(f"Invalid version format for {new['key']}. Expected X.Y.Z (e.g., 14.0.0). <a href='https://docs.frappe.io/cloud/common-issues/incompatible-app-version' target='_blank'>Learn more</a>")
...
def deploy(name, apps):
	...
	if rg.team != team.name:
		frappe.throw("Bench can only be deployed by the bench owner. <a href='https://docs.frappe.io/cloud/role-permissions' target='_blank'>Learn more</a>", exc=frappe.PermissionError)
...
def validate_app_hashes(apps: list[dict[str, str]]):
	...
	for app in apps:
		if not app.get("release") or not app.get("hash"):
			frappe.throw("Each app must have a valid release and commit hash to deploy.")
		else:
...
def validate_branch(name: str, app: str, branch: str):
	...
	frappe.throw("Could not validate branch on GitHub: " + response.text)
	return None
...
def log(name, bench, log):
	if frappe.db.get_value("Bench", bench, "group") != name:
		frappe.throw(f"Release Group '{name}' does not match this bench's Release Group.")
...
def fail_build(dn: str):
	...
	if not failed:
		frappe.throw("No running job found. It may have already completed or failed.")
...
def redeploy(name: str, dc_name: str) -> str:
	...
	if response["error"]:
		frappe.throw("Unable to redeploy this build. It may have already been redeployed or is in an invalid state.", frappe.ValidationError)
