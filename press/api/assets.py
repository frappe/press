...
def upload_asset():
	...
	if not files or "asset_file" not in files:
		frappe.throw(
			"No asset file uploaded. Please select a file to upload. "
...
def get_credentials() -> AssetStoreCredentials:
	...
	if not build_token:
		frappe.throw(
			...
			'<a href="https://docs.frappe.io/cloud/local-fc-setup" target="_blank">Learn more</a>',
	...
	if not deploy_candidate:
		frappe.throw(
			"Invalid build token used. Ensure you are using the correct token from your deploy candidate. ",
			frappe.PermissionError,
		)
	...
	if not running_build:
		frappe.throw(
			"Expired token used. Request a new build token. ",
			frappe.PermissionError,
		)
