...
def send_verification_code(domain: str, route: str = ""):
	...
	if not domain_info or domain_info.get("status") != "Active":
		frappe.throw("The domain is not active currently. Please try again after activation.<a href='https://docs.frappe.io/cloud/sites/custom-domains' target='_blank'>Learn more</a>")
	...
	if not team_info or not team_info.get("enabled"):
		frappe.throw("Your Frappe Cloud team is disabled currently. Please contact support to enable your team.")
...
def check_if_user_can_login(team_info, site_info):
	...
	if not (site_info.get("standby_for") or site_info.get("standby_for_product")):
		frappe.throw("Only SaaS sites are allowed to login to Frappe Cloud via this method.<a href='https://docs.frappe.io/cloud/simplified-login-to-frappe-cloud-dashboard' target='_blank'>Learn more</a>")
