...
class TestScheduledAutoUpdates(TestCase):
	def test_should_update_daily_positive(self):
		for obj in TEST_DATA_DAILY_TRUE:
			self.assertTrue(should_update_trigger_for_daily(frappe._dict(obj), obj["current_datetime"]), obj)

	def test_should_update_daily_negative(self):
		for obj in TEST_DATA_DAILY_FALSE:
			self.assertFalse(should_update_trigger_for_daily(frappe._dict(obj), obj["current_datetime"]), obj)

	def test_should_trigger_weekly_positive(self):
		for obj in TEST_DATA_WEEKLY_TRUE:
			self.assertTrue(should_update_trigger_for_weekly(frappe._dict(obj), obj["current_datetime"]), obj)
	...
	def test_true_last_triggered_is_none(self):
		TEST_DATA_DAILY_TRUE_WITH_NONE = list(map(set_last_triggered_to_none, TEST_DATA_DAILY_TRUE))

		TEST_DATA_WEEKLY_TRUE_WITH_NONE = list(map(set_last_triggered_to_none, TEST_DATA_WEEKLY_TRUE))

		TEST_DATA_MONTHLY_TRUE_WITH_NONE = list(map(set_last_triggered_to_none, TEST_DATA_MONTHLY_TRUE))
