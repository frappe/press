...
def _is_success_response(result: object) -> bool:
	"""Check if a backend response indicates success."""
...
def _format_error(result: object) -> str:
	"""Extract a readable error message from a backend response."""
	if isinstance(result, dict):
		return str(result.get("message") or result.get("exception") or result.get("exc") or "Unknown error")
	return str(result) if result else "Unknown error"


def _find_frappe_source(options: dict, version: str) -> str | None:
	"""Find the Frappe source name for a given version."""
	for v in options.get("versions", []):
		if v.get("name") == version:
			for app in v.get("apps", []):
				if app.get("name") == "frappe":
					return app.get("source", {}).get("name")
	return None


def _find_app_source(apps_list: list[dict], app: str, branch: str) -> str | None:
	"""Find an app source by name and branch."""
...
def _get_deploy_info(session: "CloudSession", bench_name: str) -> dict:
	...
	return session.get("press.api.bench.deploy_information", params={"name": bench_name}) or {}


def _build_apps_payload(info: dict, apps: list[str], hashes: list[str], bench_name: str) -> list[dict]:
	"""Build the apps payload for deploy_and_update."""
	payload: list[dict] = []
	for app_name, hash_val in zip(apps, hashes, strict=True):
		...
		if not app_entry:
			...
			continue
		release_name, _ = _select_release_by_prefix(app_entry.get("releases", []) or [], hash_val)
		if release_name:
			payload.append({"app": app_name, "release": release_name})
	return payload


def _resolve_sites_for_update(info: dict, sites_opt: list[str] | None, bench_name: str) -> list[dict]:
	"""Resolve site names to site dicts for update payload."""
	if not sites_opt:
		return []
...
def _prepare_bench_payload(title: str, version: str, region: str, frappe_source: str, server: str) -> dict:
	"""Prepare the payload for creating a new bench."""
	return {
		"title": title,
		"version": version,
		"cluster": region,
...
def _warn_server_name_format(server: str) -> None:
	"""Warn if server name has trailing backslash or whitespace."""
...
def _create_bench(
	...
	"""Create a new bench group via the API."""
	payload = _prepare_bench_payload(title, version, region, frappe_source, server)
	try:
		response = session.post(
			"press.api.bench.new",
			json={"bench": payload},
			message=f"[bold green]Creating bench group '{title}' for version '{version}', region '{region}', server '{server}'...",
		)
		if isinstance(response, str):
			Print.success(console, f"Successfully created bench group: {response}")
		elif _is_success_response(response):
			Print.success(console, f"Successfully created bench group: {title}")
		else:
			Print.error(console, f"Failed to create bench group: {_format_error(response)}")
	except Exception as e:
		Print.error(console, f"Request error: {e}")
...
def create_initial_deploy(
	...
	try:
		response = session.post(
			...
			json={"dt": "Release Group", "dn": name, "method": "initial_deploy", "args": None},
			message=f"[bold green]Triggering initial deploy for '{name}'...",
		)
		if _is_success_response(response):
			Print.success(console, f"Initial deploy triggered for bench group: {name}")
		else:
			Print.error(console, f"Failed to trigger initial deploy: {_format_error(response)}")
	except Exception as e:
...
def drop_bench_group(
	...
	try:
		response = session.post(
			...
			json={"doctype": "Release Group", "name": name},
			message=f"[bold red]Dropping bench group '{name}'...",
		)
		if _is_success_response(response):
			Print.success(console, f"Successfully dropped bench group: {name}")
		else:
			Print.error(console, f"Failed to drop bench group: {_format_error(response)}")
	except Exception as e:
...
def add_app(
	...
	session: CloudSession = ctx.obj
	try:
		apps_list = session.post("press.api.bench.all_apps", json={"name": bench})
		if not isinstance(apps_list, list):
			Print.error(console, "Failed to fetch apps list.")
			return

		source = _find_app_source(apps_list, app, branch)
		if not source:
		...
		response = session.post(
			...
			json={"name": bench, "source": source, "app": app},
		)
		if isinstance(response, str) and "already exists" in response:
			Print.info(console, f"App '{app}' is already added to bench '{bench}'.")
		elif _is_success_response(response):
			Print.success(console, f"Successfully added app '{app}' (branch '{branch}') to bench '{bench}'")
		else:
			Print.error(console, f"Failed to add app '{app}' to bench '{bench}': {_format_error(response)}")
	except Exception as e:
...
def remove_app(
	...
	try:
		response = session.post(
			"press.api.client.run_doc_method",
			json={"dt": "Release Group", "dn": bench, "method": "remove_app", "args": {"app": app}},
		)
		if isinstance(response, str):
			...
			Print.error(
				console, f"Failed to remove app '{app}' from bench '{bench}': {_format_error(response)}"
			)
	except Exception as e:
...
def update_app(
	...
	sites_opt: Annotated[
		list[str] | None,
		typer.Option("--site", help="Site(s) to include in update (repeat --site for multiple)"),
	] = None,
):
	session: CloudSession = ctx.obj
	...
	try:
		info = _get_deploy_info(session, bench)
		apps_payload = _build_apps_payload(info, app, hash_prefix, bench)
		if not apps_payload:
			Print.error(console, "No valid app/release pairs to update.")
			return

		selected_sites = _resolve_sites_for_update(info, sites_opt, bench)
		payload = {
			"name": bench,
			"apps": apps_payload,
		...
		result = session.post(
			...
			json=payload,
			message=f"[bold green]Updating {len(apps_payload)} app(s) on bench '{bench}' with {len(selected_sites)} site(s)...",
		)

		if _is_success_response(result):
			msg = (
				f"Update scheduled. Tracking name: {result}"
				if isinstance(result, str) and result
				else "Update scheduled successfully."
			)
			Print.success(console, msg)
		else:
			Print.error(console, f"Failed to schedule update: {_format_error(result)}")
	except Exception as e:
