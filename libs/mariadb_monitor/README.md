...
A robust health and resource monitor for MariaDB designed to detect sustained system pressure (CPU, Memory, I/O) and gracefully recover the daemon to prevent hanging queries, deadlocks, or severe performance degradation.
...
- **`critical_memory_threshold`**: If memory usage exceeds this percentage *at any moment* (without waiting for the window ratio), it immediately flags critical pressure, escalating the health checks.
- **`swap_usage_threshold`**: Threshold for sustained swap usage percentage.
...
#### Database Health
- **`stuck_query_threshold`**: Time considered before an ongoing query in specific states (like "Killed" or "Opening tables") or any general long-running query is flagged as "stuck". If the system is under pressure *and* stuck queries exist, recovery is triggered.
