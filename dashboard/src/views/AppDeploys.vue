	...
	<div>
		<Section title="Deploys" description="Deploy log of your app">
			<SectionCard class="md:w-2/3">
				<div v-if="deploys.loading == false">
					<div v-if="deploys.data.releases && deploys.data.releases.length">
						<div
							class="text-base items-center px-6 py-3 hover:bg-gray-50"
							v-for="deploy in deploys.data.releases"
							:key="deploy.name"
						>
							...
							<div class="items-center pl-6" v-if="deploy.groups">
								<div
									v-for="group in deploy.groups"
									:key="group.group"
									class="block py-1 flex justify-between items-center"
								>
									<div>
										...
										<p class="text-sm text-gray-800">
											frappe/{{ deploys.data.groups[group.group].branch }}
										</p>
									</div>
									<div class="w-32">
