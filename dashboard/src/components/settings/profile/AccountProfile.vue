<template>
	<Card title="Profile" v-if="user" class="mx-auto max-w-3xl">
		...
		<div>
			...
			<ListItem
				v-if="user.is_2fa_enabled"
				title="Recovery Code Reminders"
				subtitle="Get an email every month until you review your recovery codes"
			>
				<template #actions>
					<Switch v-model="recoveryCodeReminders" />
				</template>
			</ListItem>
			<ListItem
				class="border-t"
				...
				<template #actions>
					...
					>
						<span :class="{ 'text-red-600': teamEnabled }"
							>{{ teamEnabled ? 'Disable' : 'Enable' }}</span
						>
					</Button>
				</template>
			</ListItem>
...
export default {
	...
	computed: {
		...
		recoveryCodeReminders: {
			get() {
				return !this.user.unsubscribed_from_recovery_code_reminders
			},
			set(value) {
				this.$resources.setRecoveryCodeReminders.submit({ enabled: value })
			},
		},
	},
	resources: {
		...
		setRecoveryCodeReminders: {
			url: 'press.api.account.set_2fa_recovery_code_reminders',
			onSuccess() {
				this.reloadAccount()
			},
		},
