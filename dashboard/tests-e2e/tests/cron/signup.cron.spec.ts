...
function fetchProductTrials(): string[] {
	...
	if (!products.length) {
		throw new Error(
			'Set PRODUCT_TRIALS (comma separated) to run signup E2E tests.',
		)
	}
	return products
}
...
const PER_PRODUCT_TIMEOUT_MS = parseInt(
	process.env.SIGNUP_PER_PRODUCT_TIMEOUT_MS || '300000',
	10,
) // 5 minutes default
const INACTIVITY_LIMIT_MS = parseInt(
	process.env.SIGNUP_INACTIVITY_MS || '0',
	10,
) // disabled by default
...
async function runSignupFlow(page: Page, product: string) {
	...
	function remaining() {
		return PER_PRODUCT_TIMEOUT_MS - (Date.now() - flowStart)
	}
	function touch(label: string) {
		lastActivity = Date.now() /* lightweight marker */
	}
	function ensureTime(label: string) {
		if (remaining() <= 0) {
			throw new Error(
				`Per-product timeout (${PER_PRODUCT_TIMEOUT_MS}ms) exceeded at phase: ${label}`,
			)
		}
		if (
			...
			throw new Error(
				`Inactivity > ${INACTIVITY_LIMIT_MS}ms detected at phase: ${label}`,
			)
		}
	}
	async function runStep<T>(
		...
		const result = await Promise.race([
			...
			page.waitForTimeout(timeoutCap).then(() => {
				throw new Error(
					`Step timeout (${label}) after ${timeoutCap}ms (remaining budget exhausted or step slow)`,
				)
			}),
		])
		touch(label + ':done')
		...
		;(async () => {
			while (remaining() > 0) {
				await page.waitForTimeout(
					Math.min(5000, Math.max(1000, INACTIVITY_LIMIT_MS / 4)),
				)
				if (Date.now() - lastActivity > INACTIVITY_LIMIT_MS) {
					// Force a failure by triggering ensureTime on next step; or throw here if desired
					throw new Error(
						`Inactivity watcher: no activity for ${INACTIVITY_LIMIT_MS}ms in product ${product}`,
					)
				}
			}
		})().catch((err) =>
			console.warn('[signup.spec] inactivity watcher error', err?.message),
		)
	}

	const email = testEmail(product.toLowerCase().replace(/\s+/g, '-'))
	await runStep('goto', () =>
		page.goto(`/dashboard/signup?product=${encodeURIComponent(product)}`),
	)
	await runStep('wait-form', () =>
		page.waitForSelector('form', { timeout: Math.min(30000, remaining()) }),
	)
	await runStep('wait-email-selectors', () =>
	...
	await runStep('signup-submit-and-response', () =>
		Promise.all([
			(async () => {
				const resp = await page.waitForResponse(
					(r) =>
						r.url().includes('press.api.account.signup') && r.status() === 200,
				)
				try {
	...
	if (!accountRequestId) {
		throw new Error(
			'Signup response did not return account_request id. Cannot fetch OTP.',
		)
	}
	if (otpHelper) {
		try {
			const baseHost = new URL(process.env.BASE_URL || 'http://localhost:8010')
				.hostname
			const otpRes = await fetch(
				`${otpHelper}?account_request=${encodeURIComponent(accountRequestId)}`,
				{
			...
			if (!code) {
				console.warn(
					`[signup.spec] OTP helper did not return code (status ${otpRes.status}). Falling back to 111111.`,
				)
			}
		} catch (e) {
			console.warn(
				`[signup.spec] OTP fetch error ${(e as Error).message}. Falling back to 111111.`,
			)
		}
	} else {
		console.warn(
			'[signup.spec] OTP_HELPER_ENDPOINT not set; using fallback OTP 111111.',
		)
	}
	if (!code) {
		code = '111111'
	...
	await runStep('fill-otp', () =>
		page.getByLabel(/verification code/i).fill(code!),
	)
	await runStep('verify-otp', () =>
		Promise.all([
			page.waitForResponse(
				(r) =>
					r.url().includes('press.api.account.verify_otp') &&
					r.status() === 200,
			),
	...
	await runStep('post-verify-redirect', () =>
		page.waitForURL(/.*\/dashboard\/(setup-account|saas|create-site)\//, {
	...
	if (page.url().includes('/dashboard/setup-account/')) {
		await Promise.race([
			page
				.waitForSelector('form button:has-text("Create account")', {
					timeout: 30000,
				})
				.catch(() => null),
			page
				.waitForSelector(
					'input[name="fname"], label:has-text("First name"), label:has-text("First Name")',
					{ timeout: 30000 },
				)
				.catch(() => null),
		])
		...
		await runStep('create-account', () =>
			Promise.all([
				page.waitForURL(/.*\/dashboard\/(saas|create-site)\//, {
					timeout: Math.min(60_000, remaining()),
				}),
				(async () => {
					await page.waitForTimeout(400)
	...
	if (/\/dashboard\/create-site\/[^/]+\/setup/.test(page.url())) {
		...
		await runStep('site-input-visible', () =>
			siteInput
				.first()
				.waitFor({ state: 'visible', timeout: Math.min(15000, remaining()) }),
		)
		let currentVal = (
			await siteInput
				.first()
		...
		if (!currentVal) {
			const base =
				product
					...
					.replace(/^-+|-+$/g, '')
					.slice(0, 15) || 'site'
			currentVal = `${base}-pw${crypto.randomBytes(2).toString('hex')}`.slice(
				0,
				32,
			)
		...
		try {
			const domainEl = page
				.locator('form .flex.cursor-default')
		...
		await runStep('create-site-submit', () =>
			Promise.all([
				page.waitForResponse(
					(r) =>
						r.url().includes('press.api.client.run_doc_method') &&
						r.request().method() === 'POST' &&
						(r.request().postData() || '').includes('create_site'),
				),
				(async () => {
					await page.waitForTimeout(400)
			...
			if (host && host !== originHost && /\/app\//.test(url)) {
				if (domainSuffix && siteSubdomain) {
					...
					if (hostNoPort !== expectedFull) {
						const relaxedOk =
							hostNoPort.includes(siteSubdomain.toLowerCase()) &&
							hostNoPort.endsWith(`.${domainSuffix.toLowerCase()}`)
						if (!relaxedOk) {
							throw new Error(
								`Provisioned site host mismatch. Expected ${expectedFull} (allowing variations), got ${hostNoPort}. Details: subdomain=${siteSubdomain} domainSuffix=${domainSuffix} finalUrl=${url}`,
							)
						}
					}
				}
		...
		if (!success) {
			throw new Error(
				`Did not observe redirect to provisioned site app. Last URL: ${activePage.url()}`,
			)
		}
