    ...
    <Dialog :class="showDialog" :options="{ title: 'Add Payment Gateway', size: 'lg' }">
      <template #body-content>
        ...
        <div class="mt-4 flex w-full bg-red-300 items-center justify-center">
          <Button @click="savePaymentGateway" variant="solid" class="justify-center w-full font-bold">Save</Button>
        </div>
...
if (missingFields.length > 0) {
  return `The following fields are missing: ${missingFields.join(', ')}`;
        ...
        async onSuccess(data) {
            console.log("params", this.currencyInput, this.gatewayName, this.integrationLogo, this.gatewayInput, this.controllerInput, this.URL, this.apiKey, this.apiSecret, this.taxesAndCharges);
          if (data) {
  ...
  methods: {
    ...
    async savePaymentGateway() {
      try {
        ...
        const response = await this.$resources.createPaymentGatewaySettings.submit();
        this.$emit('closeDialog');
      } catch (error) {
        this.$toast.error(`Error saving Payment Gateway settings: ${error.message}`);
      }
    },
    ...
    async fetchGatewayControllers(gatewaySetting){
        try{
            ...
            this.$toast.error(`Error fetching gateway controllers: ${error.message}`);
        }
    },
  },
