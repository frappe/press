...
methods: {
    ...
    get_midtrans_card_token(){
        ...
        var options = {
            onSuccess:(response)=>{
                ...
                response["card_name"] = this.cardName
                response["expiry_month"] = this.cardMonth
                response["expiry_year"] = this.cardYear
                response["team"] = this.$team.doc.name
                response["last_4"] = this.cardNumber.slice(-4)
    ...
    resources : {
        CreatePaymentMethod() {
            return {
                url: 'optibizpro.utils.create_midtrans_payment_method',
                onSuccess() {
                    this.$emit('success');
                    this.$emit('complete');
