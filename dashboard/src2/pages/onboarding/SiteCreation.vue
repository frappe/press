...
export default {
	...
	data() {
		return {
			agentJobName: this.$route.query.agentJob,
			currentBuildStep: 'Preparing to install...',
			progressCount: 0
		};
	},
	mounted() {
		this.$socket.emit('doc_subscribe', 'Site Group Deploy', this.agentJobName);

		this.$socket.on('doc_update', () => {
			this.$resources.agentJob.reload();
		});
	},
	resources: {
		app() {
		...
		agentJob() {
			return {
				...
				onSuccess: doc => {
					// check latest step and update progress
	...
	computed: {
		failure() {
			if (!this.$resources.agentJob.doc?.steps) return false;

			return this.$resources.agentJob.doc.steps.some(
		...
		// 		].indexOf(status);
		// 	};
		// 	const status = this.$resources.agentJob?.doc?.status;
